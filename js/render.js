...
function getData(months){
  ...
  if(months<=3) return {stg:'초기 영아기 (2~3개월)',em:'🌸',qt:'첫 미소는 세상에 보내는 첫 번째 사랑의 신호입니다.',g:['#2A5040','#4A8A68'],role:'부모',
  ...
  if(months<=12) return {stg:'후기 영아기 (7~12개월)',em:'🌻',qt:'첫 걸음, 첫 말. 이 모든 처음을 목격하는 것이 부모의 특권입니다.',g:['#305545','#4A8060'],role:'부모',
  ...
  if(months<=24) return {stg:'걸음마기 (13~24개월)',em:'🐾',qt:'"싫어!"는 건강한 자아의 신호입니다. 이 에너지를 꺾지 말고 방향을 잡아주세요.',g:['#355545','#5A7A65'],role:'부모',
  ...
  if(months<=36) return {stg:'유아기 초반 (만 2~3세)',em:'🌈',qt:'아이의 상상 속 세계를 함께 믿어주세요. 그 세계가 창의성의 토대입니다.',g:['#3A5A40','#5A7A58'],role:'부모',
  ...
  if(y<=6) return {stg:'유아기 (만 4~6세)',em:'🎨',qt:'이 나이의 놀이는 그냥 노는 것이 아닙니다. 놀이가 곧 공부입니다.',g:['#5A4A35','#8A7050'],role:'부모',
  ...
  if(y<=12) return {stg:'학령기 (7~12세)',em:'📚',qt:'학교는 지식을 배우는 곳이지만, 가정은 삶을 배우는 곳입니다.',g:['#504535','#7A6A50'],role:'부모',
  ...
  if(y<=29) return {stg:'청년기 (19~29세)',em:'🦋',qt:'이제 아이는 어른이 되었습니다. 부모의 역할은 지지자로 바뀌어야 합니다.',g:['#2A4550','#4A6A7A'],role:'부모',
  ...
  if(y<=44) return {stg:'성인기 (30~44세)',em:'🌳',qt:'뿌리가 깊어야 바람에 흔들리지 않습니다. 지금 심는 것들이 평생의 기반입니다.',g:['#3A4A35','#5A6A4A'],role:'부모',
  ...
  if(y<=59) return {stg:'중년기 (45~59세)',em:'🍂',qt:'중년은 인생의 정점이자 다음 계절을 준비하는 수확의 시간입니다.',g:['#4A3A30','#7A5A45'],role:'부모',
  ...
  if(y<=74) return {stg:'노년기 (60~74세)',em:'🌿',qt:'긴 여정을 걸어온 지혜는 가장 값진 유산입니다.',g:['#2A3A4A','#4A5A6A'],role:'가족·보호자',
  ...
  return {stg:'후기 노년기 (75세 이상)',em:'🌾',qt:'긴 삶을 걸어온 이가 존재 자체로 가족에게 선물입니다.',g:['#2A2A3A','#4A4A5A'],role:'가족·보호자',
