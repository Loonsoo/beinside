...
const PP_STAGES = [
  { maxDay: 14, label: '출산 후 첫 2주', lines: [
    '눈물이 나고 기분이 자주 오르내리는 건 많은 산모가 겪어요(베이비 블루스). 보통 2주 안에 옅어져요.',
    '아기가 잘 때 같이 자는 게 먼저예요. 집안일은 미뤄도 돼요.',
    '38℃ 이상 열이나 한 시간에 생리대를 하나 넘게 적시는 출혈이 있으면 바로 병원에 가세요.',
  ]},
  { maxDay: 42, label: '출산 후 3~6주', lines: [
    '우울한 기분이 2주 넘게 이어지면 베이비 블루스가 아니라 산후우울일 수 있어요. 아래 "마음 신호 확인"을 해 보세요.',
    '산후 검진 때 기분 이야기도 해도 돼요. 몸만 보는 자리가 아니에요.',
  ...
  { maxDay: 90, label: '출산 후 6주~3개월', lines: [
    ...
    '아기가 계속 울면 안전한 곳에 눕히고 잠시 자리를 떠도 괜찮아요.',
  ]},
...
function ppRenderAge() {
  ...
  if (days < 0) {
    ...
    html = '<h2 class="pp-card-title">아기가 태어난 지 <strong>' + week + '주째</strong>예요</h2>'
      ...
      + '<ul class="pp-list">' + stage.lines.map(l => '<li>' + esc(l) + '</li>').join('') + '</ul>';
  }
  el.innerHTML = html
...
function ppConnectHTML() {
  return '<div class="pp-connect">'
    + '<a class="pp-call pp-call--primary" href="' + helplineTel('suicide') + '" data-umami-event="pp-connect" data-umami-event-type="call-109">📞 109에 전화하기 <span>무료 · 24시간</span></a>'
    + '<a class="pp-call" href="' + helplineSms('suicide') + '" data-umami-event="pp-connect" data-umami-event-type="sms-109">💬 109에 문자 보내기</a>'
    + '<a class="pp-call" href="' + MADLAN_URL + '" target="_blank" rel="noopener noreferrer" data-umami-event="pp-connect" data-umami-event-type="madlan">💬 카카오톡 마들랜 상담</a>'
    + '<a class="pp-call" href="' + helplineTel('mental') + '" data-umami-event="pp-connect" data-umami-event-type="call-15770199">📞 1577-0199 정신건강상담전화</a>'
    + '<p class="pp-script">처음엔 <strong>"아기 낳고 요즘 많이 힘들어요"</strong>라고만 해도 돼요.<br>나나 아기가 지금 위험하다면 <a href="tel:119">119</a>에 전화해 주세요.</p>'
    + '</div>';
}

const PP_MOOD_REPLY = {
  ok: () => '<p class="pp-reply">좋아요. 힘든 날이 오면 아래 <strong>바로 연락해야 하는 신호</strong>부터 보세요.</p>',
...
function toggleHomeOther() {
  ...
  if (!other.hidden) {
    ...
    setTimeout(() => other.scrollIntoView({ behavior: 'smooth', block: 'start' }), 60);
  }
}
